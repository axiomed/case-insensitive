...
inductive Assertion where
  ...
  | failure (message : String) (expected : String) (resukt : String) : Assertion
  deriving Repr
...
def Assertion.run (assertion : Assertion) : IO Bool := do
  ...
  | success message =>
      IO.println (green ++ "[PASS] " ++ message ++ reset)
      return true
...
def run (results : List Assertion) :=
  ...
  do
    ...
    if failed > 0 then IO.println s!"\nTests: {red}{failed} failed{reset}, {green}{passed} passed{reset}, {total} total\n" else IO.println "\nAll tests have passed!\n"
